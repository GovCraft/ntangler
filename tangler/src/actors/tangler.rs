...
impl Tangler {
    ...
    pub(crate) async fn init(tangler_config: TanglerConfig) -> anyhow::Result<(Context, Context)> {
        ...
        let actor_config = ActorConfig::new(Arn::with_root("tangler")?, None, Some(broker.clone()))?;

        let actor_context = akton.spawn_actor_with_setup::<Tangler>(actor_config, |mut actor| Box::pin(
            async move {
                ...
                actor
                    ...
                    .on_before_stop_async(|actor| {
                        ...
                        Box::pin(async move {
                            broker.suspend_actor().await.expect("Failed to shut down broker");
                        })
                    });
